...
func CreateAi(l *littlebee.Ai) (*littlebee.Ai, error) {
...
func RetrieveAi(aiId int32) (*littlebee.Ai, error) {
	r := &littlebee.Ai{}
	if err := database.DB.First(r.Signal, aiId).Error; err != nil {
		return nil, err
	}
	if err := database.DB.First(r.SignalAcquisition, aiId).Error; err != nil {
		return nil, err
	}
	if err := database.DB.First(r.SignalAi, aiId).Error; err != nil {
		return nil, err
	}
	if err := database.DB.Where("signal_id = ?", aiId).Find(r.SignalThreshold).Error; err != nil {
		return nil, err
	}
	return r, nil
}

func UpdateAi(l *littlebee.Ai) error {
	err := database.DB.Transaction(func(tx *gorm.DB) error {
		if l.SignalBindConfigure != nil {
...
func DeleteAiWithFlag(l *littlebee.Ai) error {
...
func DeleteAi(l *littlebee.Ai) error {
	err := database.DB.Transaction(func(tx *gorm.DB) error {
