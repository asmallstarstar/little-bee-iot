...
func CreateOutput(l *littlebee.Output) (*littlebee.Output, error) {
...
func RetrieveOutput(outputId int32) (*littlebee.Output, error) {
	r := &littlebee.Output{}
	if err := database.DB.First(r.Signal, outputId).Error; err != nil {
		return nil, err
	}
	if err := database.DB.First(r.SignalAcquisition, outputId).Error; err != nil {
		return nil, err
	}
	return r, nil
}

func UpdateOutput(l *littlebee.Output) error {
	err := database.DB.Transaction(func(tx *gorm.DB) error {
		if l.SignalBindConfigure != nil {
...
func DeleteOutputWithFlag(l *littlebee.Output) error {
...
func DeleteOutput(l *littlebee.Output) error {
	err := database.DB.Transaction(func(tx *gorm.DB) error {
