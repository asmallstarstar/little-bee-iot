...
func CreateVdi(c *gin.Context) {
	...
	if err := serialization.UnmarshalFromJson(string(buf), l); err != nil {
		...
		r, err := vdi.CreateVdi(l, userId.(int32))
		if err != nil {
...
func RetrieveVdi(c *gin.Context) {
	lang := c.Request.FormValue("lang")
	acceptLanguage := c.Request.Header.Get("Accept-Language")
	c.Writer.Header().Set("Content-Type", "application/json")

	if vdiId, err := strconv.Atoi(c.Param("vdi-id")); err != nil {
		...
		r, err := vdi.RetrieveVdi(int32(vdiId))
		if err != nil {
...
func UpdateVdi(c *gin.Context) {
	...
	if err := serialization.UnmarshalFromJson(string(buf), l); err != nil {
		...
		userId, _ := c.Get("userId")
		err := vdi.UpdateVdi(l, userId.(int32))
		if err != nil {
			c.String(http.StatusInternalServerError, "%s", response.ResponseErrorJsonByErrorCode(myerror.ERROR_WRITE_DATABASE, nil, lang, acceptLanguage, err))
		} else {
...
func DeleteVdi(c *gin.Context) {
	...
	if vdiId, err := strconv.Atoi(c.Param("vdi-id")); err != nil {
		...
		userId, _ := c.Get("userId")
		err := vdi.DeleteVdi(vdiId, userId.(int32))
		if err != nil {
			c.String(http.StatusInternalServerError, "%s", response.ResponseErrorJsonByErrorCode(myerror.ERROR_WRITE_DATABASE, nil, lang, acceptLanguage, err))
		} else {
			c.String(http.StatusOK, "%s", response.ResponseSuccessJson(nil))
...
func DeleteVdiWithFlag(c *gin.Context) {
	lang := c.Request.FormValue("lang")
	acceptLanguage := c.Request.Header.Get("Accept-Language")
	c.Writer.Header().Set("Content-Type", "application/json")
	...
	if vdiId, err := strconv.Atoi(c.Param("vdi-id")); err != nil {
		...
		userId, _ := c.Get("userId")
		err := vdi.DeleteVdiWithFlag(vdiId, userId.(int32))
		if err != nil {
			c.String(http.StatusInternalServerError, "%s", response.ResponseErrorJsonByErrorCode(myerror.ERROR_WRITE_DATABASE, nil, lang, acceptLanguage, err))
		} else {
			c.String(http.StatusOK, "%s", response.ResponseSuccessJson(nil))
