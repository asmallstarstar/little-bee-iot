...
func CreateSi(c *gin.Context) {
	...
	if err := serialization.UnmarshalFromJson(string(buf), l); err != nil {
		...
		r, err := si.CreateSi(l, userId.(int32))
		if err != nil {
...
func RetrieveSi(c *gin.Context) {
	lang := c.Request.FormValue("lang")
	acceptLanguage := c.Request.Header.Get("Accept-Language")
	c.Writer.Header().Set("Content-Type", "application/json")

	if siId, err := strconv.Atoi(c.Param("si-id")); err != nil {
		...
		r, err := si.RetrieveSi(int32(siId))
		if err != nil {
...
func UpdateSi(c *gin.Context) {
	...
	if err := serialization.UnmarshalFromJson(string(buf), l); err != nil {
		...
		userId, _ := c.Get("userId")
		err := si.UpdateSi(l, userId.(int32))
		if err != nil {
			c.String(http.StatusInternalServerError, "%s", response.ResponseErrorJsonByErrorCode(myerror.ERROR_WRITE_DATABASE, nil, lang, acceptLanguage, err))
		} else {
...
func DeleteSi(c *gin.Context) {
	...
	if siId, err := strconv.Atoi(c.Param("si-id")); err != nil {
		...
		userId, _ := c.Get("userId")
		err := si.DeleteSi(siId, userId.(int32))
		if err != nil {
			c.String(http.StatusInternalServerError, "%s", response.ResponseErrorJsonByErrorCode(myerror.ERROR_WRITE_DATABASE, nil, lang, acceptLanguage, err))
		} else {
			c.String(http.StatusOK, "%s", response.ResponseSuccessJson(nil))
...
func DeleteSiWithFlag(c *gin.Context) {
	lang := c.Request.FormValue("lang")
	acceptLanguage := c.Request.Header.Get("Accept-Language")
	c.Writer.Header().Set("Content-Type", "application/json")
	...
	if siId, err := strconv.Atoi(c.Param("si-id")); err != nil {
		...
		userId, _ := c.Get("userId")
		err := si.DeleteSiWithFlag(siId, userId.(int32))
		if err != nil {
			c.String(http.StatusInternalServerError, "%s", response.ResponseErrorJsonByErrorCode(myerror.ERROR_WRITE_DATABASE, nil, lang, acceptLanguage, err))
		} else {
			c.String(http.StatusOK, "%s", response.ResponseSuccessJson(nil))
