<template>
  <el-scrollbar :max-height="maxHeight">
    <el-collapse v-model="activeNames" >
      ...
      <el-collapse-item :title="$t('language.signal.SIGNAL_VIDEO_PROPERTY')" name="videoProperty">
        <signal-video :signal-id="signalId" :operated-type="operatedType" ref="signalVideoRef"></signal-video>
      </el-collapse-item>
    </el-collapse>
    <div class="operate-group">
      <el-button type="danger" size="small" @click="handleDelete" v-if="operatedType!=operatedTypeEnum.OPERATED_TYPE_CREATE && actionsStore.isPermission(['delete-signal-ai'])">{{$t('language.common.DELETE')}}</el-button>
      <el-button type="primary" size="small" @click="handleConfirm" v-if="actionsStore.isPermission(['create-signal-ai','update-signal-ai'])">{{$t('language.common.CONFIRM')}}</el-button>
    </div>
  </el-scrollbar>
</template>
...
import SignalVideo from "@/components/setting/signal/base/SignalVideo.vue";
...
import {createVideo, deleteVideo, updateVideo} from "@/api/video";
...
function handleDelete() {
  ...
  ElMessageBox.confirm(t('language.common.DELETE_TITLE')+t('language.common.SIGNAL')+": "+signal.signalName, t('language.common.DELETE_WARNING'), {
    confirmButtonText: t('language.common.CONFIRM_BUTTON_TEXT'), cancelButtonText: t('language.common.CANCEL_BUTTON_TEXT'), type: 'warning',})
      .then(() => {
        deleteVideo(signalId.value).then(x=>{
          if(x.data.isSuccess){
            ElMessage({
              showClose: false,
...
function handleConfirm() {
  ...
  obj.signalVideo = signalVideoRef.value.getVideoAcquisition()
  const configure = signalVideoRef.value.getMetadataAttribute()
  if(configure){
    obj.videoBindConfigure = configure
  }
  let fn = createVideo
