<template>
  <el-scrollbar :max-height="maxHeight">
    <el-collapse v-model="activeNames" >
      ...
      <el-collapse-item :title="$t('language.signal.SIGNAL_SI_PROPERTY')" name="siProperty">
        <signal-si :signal-id="signalId" :operated-type="operatedType" ref="signalSiRef"></signal-si>
      </el-collapse-item>
    </el-collapse>
    <div class="operate-group">
      <el-button type="danger" size="small" @click="handleDelete" v-if="operatedType!=operatedTypeEnum.OPERATED_TYPE_CREATE && actionsStore.isPermission(['delete-signal-si'])">{{$t('language.common.DELETE')}}</el-button>
      <el-button type="primary" size="small" @click="handleConfirm" v-if="actionsStore.isPermission(['create-signal-si','update-signal-si'])">{{$t('language.common.CONFIRM')}}</el-button>
    </div>
  </el-scrollbar>
</template>
...
import SignalSi from "@/components/setting/signal/base/SignalSi.vue";
...
import {createSi, deleteSi, updateSi} from "@/api/si";
...
function handleDelete() {
  ...
  ElMessageBox.confirm(t('language.common.DELETE_TITLE')+t('language.common.SIGNAL')+": "+signal.signalName, t('language.common.DELETE_WARNING'), {
    confirmButtonText: t('language.common.CONFIRM_BUTTON_TEXT'), cancelButtonText: t('language.common.CANCEL_BUTTON_TEXT'), type: 'warning',})
      .then(() => {
        deleteSi(signalId.value).then(x=>{
          if(x.data.isSuccess){
            ElMessage({
              showClose: false,
...
function handleConfirm() {
  ...
  obj.signalSi = signalSiRef.value.getSignalSi()
